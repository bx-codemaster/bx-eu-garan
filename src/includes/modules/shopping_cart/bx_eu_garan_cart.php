<?php
/**
 * BX EU Garan - Shopping Cart Module
 *
 * This module enriches cart product rows with graphics-only EU warranty/guarantee
 * artifacts used by storefront templates:
 * - legal guarantee button image (linked to legal content)
 * - GARAN small label image/SVG (linked to guarantee details)
 *
 * Integration points:
 * - modified shopping cart module loader (`get_products` hook)
...
class bx_eu_garan_cart
{
  ...
  /** @var string Configuration key prefix (e.g. MODULE_SHOPPING_CART_BX_...). */
  public $name;

  /** @var string Language key for the module title. */
  public $title;
  ...
  /** @var string|null Legacy placeholder, kept for compatibility. */
  public $configuration_table;

  /**
  * Initializes module identifiers and runtime defaults.
  *
  * Reads configuration constants provided by modified core bootstrap.
  */
  ...
  /**
   * Hook: enriches cart product data with EU GARAN graphics.
   *
   * @param array<string, mixed> $products_data Existing product payload.
   * @param array<string, mixed> $product Source product row from cart context.
   * @param mixed $contents Cart contents context (unused, hook signature).
   *
   * @return array<string, mixed> Enriched product payload.
   */
  public function get_products($products_data, $product, $contents)
  {
    $products_id = isset($product['products_id']) ? (int)$product['products_id'] : 0;
    if ($products_id < 1) {
      return $products_data;
    }

    $context = $this->get_context_by_product_id($products_id);
  ...
  public function get_context_by_product_id($products_id)
  {
    ...
    $context['legal_label_btn'] = '<a id="bx_eu_garan_legal_label_btn" href="'.$legal_qr_url.'" target="_blank" rel="noopener nofollow">'
    ...
    if ($show_warranty_label) {
        ...
        if ($small_dom->loadXML($small_template)) {
          ...
          if ($small_svg_node instanceof DOMElement && strtolower($small_svg_node->tagName) === 'svg') {
            ...
            if ($svg_markup !== false && $svg_markup !== '') {
              $label_small = '<div class="bx-eu-garan-label-small" style="max-width: 140px;">'
                             . $svg_markup
      ...
      if ($label_small === '') {
        $label_small = '<div class="bx-eu-garan-label-small" style="max-width: 140px;">'
                      .'  <img src="'.$legal_label_path.'garan.svg" alt="" width="140" />'
                      .'  <div class="bx-eu-garan-label-big"></div>'
                      .'</div>';
      }
